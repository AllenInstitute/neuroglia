...
from neuroglia.calcium.oasis import constrained_oasisAR1, oasisAR1
...
def deconvolve(y, g=(None,), sn=None, b=None, b_nonneg=True,
    ...
    optimize_g : int, optional, default 0
        Number of large, isolated events to consider for optimizing g, e.g. 5.
...
def estimate_parameters(y, p=2, range_ff=[0.25, 0.5], method='mean',
                        lags=10, fudge_factor=1., nonlinear_fit=False):
    """
    ...
    nonlinear_fit : bool, optional, default False
        if True explicit nonlinear fit of autocovariance C(t) as function of t
        else linear fit of autocovariance recursion C(t+1) = A C(t)
    """

    sn = estimate_noise(y, range_ff, method)
...
def estimate_noise(y, range_ff=[0.25, 0.5], method='mean'):
    """
    ...
    * Pnevmatikakis et al, Neuron 2016
    ...
    ind = np.logical_and(ff > range_ff[0], ff < range_ff[1])
    Pxx_ind = Pxx[ind]
    sn = {
